...
// Commission calc (Strictly @ 1.5/- Per Meter for Trading)
...
// ─── TRADING TAB ──────────────────────────────────────────────
function TradingTab({data,onAdd,onAddPay,onDel}) {
...
// ─── AGENCY TAB ───────────────────────────────────────────────
function AgencyTab({data,onAdd,onAddPay,onDel}) {
  const [view,setView]=useState("sales");
  const [search,setSearch]=useState("");
    ...
    <div>
      ...
      <div style={{background:"#fff",borderRadius:12,padding:12,marginBottom:12,boxShadow:"0 1px 8px rgba(0,0,0,0.06)",borderLeft:`4px solid ${view==="trading"?"#2980B9":"#27AE60"}`}}>
        ...
        <div style={{fontSize:22,fontWeight:900,color:view==="trading"?"#2980B9":"#27AE60"}}>
          ₹{fmt(view==="trading" ? tradingEntries.reduce((a,e)=>a+e.net,0) : agencyEntries.reduce((a,e)=>a+e.net,0))}
        </div>
      </div>
...
// ─── AGING TAB (BILL-WISE TRACKING ADDED) ─────────────────────
function AgingTab({data}) {
  ...
  function buildAging(sales, payments) {
    ...
    sales.forEach(s=>{
      ...
      map[s.customerId].invoices.push({date:s.date, amount:+s.amount||0, id:s.id, ref:s.productName});
    });
    
    const paidMap={};
    payments.forEach(p=>{ paidMap[p.customerId]=(paidMap[p.customerId]||0)+(+(p.netAmount||p.amount)||0); });
    ...
    return Object.entries(map).map(([id,v])=>{
      ...
      sorted.forEach(inv=>{
        ...
        unpaid.push({ date: inv.date, amount: amt, days, ref: inv.ref });
      ...
      return {id, name:v.name, ...buckets, total, unpaid};
  ...
  // Master WA for all parties
  const buildAllAgingWA = () => {
    const d = new Date().toLocaleDateString("en-IN");
    let msg = `🧵 *${view==="trading"?"Trading":"Agency"} Bill-Wise Outstanding Report*\n📅 ${d}\n\n`;
    entries.forEach(e => {
        msg += `👤 *${e.name}*: ₹${fmt(e.total)}\n`;
        e.unpaid.forEach(inv => {
            msg += `   └ ${fmtD(inv.date)}: ₹${fmt(inv.amount)} (${inv.days} days)\n`;
        });
        msg += `\n`;
    });
    msg += `*Total Market Outstanding: ₹${fmt(totals.total||0)}*`;
    return msg;
  };

  // Individual WA for specific customer
  const buildPartyAgingWA = (e) => {
    const d = new Date().toLocaleDateString("en-IN");
    let msg = `🧵 *Fabric Business*\n📅 ${d}\n\nDear *${e.name}*,\n\nHere is your pending bill-wise outstanding:\n\n`;
    e.unpaid.forEach(inv => {
        msg += `🔹 ${fmtD(inv.date)} - ${inv.ref}: ₹${fmt(inv.amount)} (${inv.days} days)\n`;
    });
  ...
  return (
    <div>
      ...
      <div style={{background:"linear-gradient(135deg,#0F1923,#1A3A5C)",borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <button onClick={()=>waOpen("",buildAllAgingWA())}
      ...
      {entries.map(e=>(
        <div key={e.id} style={{background:"#fff",borderRadius:12,padding:"12px 14px",marginBottom:10,boxShadow:"0 1px 8px rgba(0,0,0,0.06)"}}>
          ...
          <button onClick={()=>waOpen(phoneMap[e.id]||"",buildPartyAgingWA(e))}
            style={{marginTop:8,background:"#E8FBF0",color:"#25D366",border:"1px solid #25D366",borderRadius:8,padding:"5px 14px",fontSize:11.5,fontWeight:700,cursor:"pointer",width:"100%"}}>
            📲 Send Bill-Wise Reminder via WhatsApp
          </button>
        </div>
      ))}
    </div>
