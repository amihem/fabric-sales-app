...
export default function App() {
  ...
  if(!data) return <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#0F1923",color:"#E8C97E",fontSize:18}}>Loading...</div>;
  ...
  return (
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#F2F4F8",minHeight:"100vh",maxWidth:480,margin:"0 auto",fontSize:"15px"}}>

      {/* HEADER */}
      <div style={{background:"linear-gradient(135deg,#0F1923 0%,#1A3A5C 100%)",padding:"16px",position:"sticky",top:0,zIndex:100,boxShadow:"0 4px 20px rgba(0,0,0,0.3)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:10,letterSpacing:2,color:"#E8C97E",fontWeight:600}}>🧵 FABRIC BUSINESS</div>
            <div style={{fontSize:22,fontWeight:900,color:"#fff"}}>Sales Manager</div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>exportBackup(data)} style={{padding:"8px 12px",background:"rgba(255,255,255,0.15)",color:"#fff",border:"none",borderRadius:8,fontSize:13}}>📤 Backup</button>
            <label style={{padding:"8px 12px",background:"rgba(255,255,255,0.15)",color:"#fff",borderRadius:8,fontSize:13}}>
      ...
      {/* TABS */}
      <div style={{background:"#1A3A5C",display:"flex",overflowX:"auto",position:"sticky",top:68,zIndex:99}}>
        {TABS.map(t => (
          <button key={t} onClick={()=>setTab(t)}
            style={{
              flex:"1 1 auto", padding:"14px 8px", fontSize:14, fontWeight: tab===t?800:600,
              color: tab===t ? "#E8C97E" : "rgba(255,255,255,0.7)", background:"none", border:"none",
              borderBottom: tab===t ? "4px solid #E8C97E" : "none"
            }}>
            {t}
          </button>
        ))}
      ...
      <div style={{padding:16, paddingBottom:100}}>
        {tab==="Dashboard" && <Dashboard {...{data,totTradingSale,totAgencySale,totTradingOut,totAgencyOut,totTradingComm,totAgencyComm}} />}
        {tab==="Trading" && <TradingTab data={data} onAdd={()=>setModal({type:"tradingSale"})} onAddPay={()=>setModal({type:"tradingPayment"})} onDel={del} />}
        {tab==="Agency" && <AgencyTab data={data} onAdd={()=>setModal({type:"agencySale"})} onAddPay={()=>setModal({type:"agencyPayment"})} onDel={del} />}
        ...
        {tab==="Reports" && <ReportsTab data={data} tradingOut={tradingOut} agencyOut={agencyOut} />}
      </div>

      {/* Modals & Toast (kept same as original) */}
      {/* ... (All modals remain unchanged for brevity) ... */}

      {toast && <div style={{position:"fixed",bottom:90,left:"50%",transform:"translateX(-50%)",background:toast.err?"#B03A2E":"#0F1923",color:"#E8C97E",padding:"12px 24px",borderRadius:30,zIndex:999}}>{toast.msg}</div>}
    </div>
  );
}

// Enhanced Aging Tab with Customer-wise Outstanding + Ageing
function AgingTab({data, tradingOut, agencyOut}) {
  const [view, setView] = useState("trading");
  const todayMs = new Date().setHours(0,0,0,0);

  const buildAging = (sales, payments, outData) => {
    const map = {};
    sales.forEach(s => {
  ...
  return (
    <div>
      <div style={{marginBottom:16}}>
        <button onClick={()=>setView("trading")} style={{marginRight:8,padding:"10px 16px",fontWeight:view==="trading"?700:500,background:view==="trading"?"#0F1923":"#fff",color:view==="trading"?"#E8C97E":"#333",borderRadius:8}}>Trading</button>
        <button onClick={()=>setView("agency")} style={{padding:"10px 16px",fontWeight:view==="agency"?700:500,background:view==="agency"?"#0F1923":"#fff",color:view==="agency"?"#E8C97E":"#333",borderRadius:8}}>Agency</button>
      </div>

      <div style={{background:"linear-gradient(135deg,#0F1923,#1A3A5C)",color:"#fff",padding:20,borderRadius:16,marginBottom:20}}>
        <div style={{fontSize:15}}>Total Outstanding</div>
        <div style={{fontSize:28,fontWeight:900}}>₹{fmt(totals.total||0)}</div>
      </div>

      {entries.map(e => (
        <div key={e.id} style={{background:"#fff",borderRadius:16,padding:18,marginBottom:16,boxShadow:"0 4px 16px rgba(0,0,0,0.1)"}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:17,fontWeight:700}}>
            <div>{e.name}</div>
            <div style={{color:"#C0392B"}}>₹{fmt(e.total)}</div>
          </div>
          <div style={{color:"#666",marginTop:4}}>Total Due: ₹{fmt(e.totalDue)}</div>

          <div style={{marginTop:16}}>
            {buckets.map(b => e[b.key] > 0 && (
              <div key={b.key} style={{display:"flex",alignItems:"center",marginBottom:10}}>
                <div style={{width:90,color:b.color,fontWeight:600}}>{b.label}</div>
                <div style={{flex:1,height:8,background:"#eee",borderRadius:4,margin:"0 12px"}}>
                  <div style={{width: `${(e[b.key]/e.total)*100}%`, height:"100%", background:b.color, borderRadius:4}} />
                </div>
          ...
          {e.b120 > 0 && (
            <button onClick={()=>waOpen(phoneMap[e.id], `Dear \( {e.name}, ₹ \){fmt(e.b120)} is overdue (120+ days). Total: ₹${fmt(e.total)}`)}
              style={{marginTop:12,width:"100%",padding:14,background:"#FADBD8",color:"#922B21",border:"none",borderRadius:12,fontWeight:700}}>
              📲 Urgent Reminder
            </button>
          )}
        </div>
